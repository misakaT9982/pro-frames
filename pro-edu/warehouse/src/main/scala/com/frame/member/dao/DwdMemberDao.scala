package com.frame.member.dao

import org.apache.spark.sql.SparkSession

/**
 * mito
 * pro-frames: com.frame.member.dao
 * 2020-12-08 22:15:43
 */
object DwdMemberDao {
    def getDwdMember(sparkSession: SparkSession) = {
        sparkSession.sql(
            """
              |select
              |   uid,ad_id,email,fullname,iconurl,lastlogin,mailaddr,memberlevel,
              |   password,phone,qq,register,regupdatetime,unitname,userip,zipcode,dt,dn
              |   from dwd.dwd_member
              |""".stripMargin)
    }
    ...
    def getDwdMemberRegType(sparkSession: SparkSession) = {
        sparkSession.sql(
            """
              |select
              |   uid,appkey,appregurl,bdp_uuid,createtime as reg_createtime,
              |   domain,isranreg,regsource,regsourcename,websiteid as siteid,dn
